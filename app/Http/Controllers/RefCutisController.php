...
class RefCutisController extends Controller
{
    /**
    ...
    public function indexSearchAll(Request $request)
    {
        ...
        $resource = RefCutis::where('nome','LIKE', '%'. $request->input('text') .'%')
        ->orderBy('nome')->get();
        $response = RestResponse::createSuccessResponse($resource, 200);
        return response()->json($response->toArray(), $response->getStatusCode());
    }
    ...
    {
        $this->authorize('update', RefCutis::class);
    ...
    {
        $this->authorize('delete', RefCutis::class);
        ...
        $response = RestResponse::createSuccessResponse([], 204, 'Cor de pele excluída com sucesso.');
        return response()->json($response->toArray(), $response->getStatusCode());
    }

    private function buscarRecurso($id)
    {
        $resource = RefCutis::find($id);

        // Verifique se o modelo foi encontrado e não foi excluído
        if (!$resource || $resource->trashed()) {
            // Gerar um log
            $codigo = 404;
            $mensagem = "O ID da cor de pele informada não existe ou foi excluída.";
    ...
    {
        $query = RefCutis::where('nome', $request->input('nome'));
        ...
        if ($query->exists()) {
            $codigo = 409;
            $mensagem = "O nome da cor de pele informada já existe.";
