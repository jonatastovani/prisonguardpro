...
class RefOlhoCorController extends Controller
{
    /**
    ...
    {
        $this->authorize('update', RefOlhoCor::class);
    ...
    {
        $this->authorize('delete', RefOlhoCor::class);
        ...
        $response = RestResponse::createSuccessResponse([], 204, 'Cor de olhos excluída com sucesso.');
        return response()->json($response->toArray(), $response->getStatusCode());
    }

    private function buscarRecurso($id)
    {
        $resource = RefOlhoCor::find($id);

        // Verifique se o modelo foi encontrado e não foi excluído
        if (!$resource || $resource->trashed()) {
            // Gerar um log
            $codigo = 404;
            $mensagem = "O ID da cor de olhos informada não existe ou foi excluída.";
    ...
    {
        $query = RefOlhoCor::where('nome', $request->input('nome'));
        ...
        if ($query->exists()) {
            $codigo = 409;
            $mensagem = "O nome da cor de olhos informada já existe.";
