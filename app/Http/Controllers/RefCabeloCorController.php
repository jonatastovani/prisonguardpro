...
class RefCabeloCorController extends Controller
{
    /**
    ...
    public function indexSearchAll(Request $request)
    {
        ...
        $resource = RefCabeloCor::where('nome','LIKE', '%'. $request->input('text') .'%')
        ->orderBy('nome')->get();
        $response = RestResponse::createSuccessResponse($resource, 200);
        return response()->json($response->toArray(), $response->getStatusCode());
    }
    ...
    {
        $this->authorize('update', RefCabeloCor::class);
    ...
    {
        $this->authorize('delete', RefCabeloCor::class);
        ...
        $response = RestResponse::createSuccessResponse([], 204, 'Cor de cabelo excluída com sucesso.');
        return response()->json($response->toArray(), $response->getStatusCode());
    }

    private function buscarRecurso($id)
    {
        $resource = RefCabeloCor::find($id);

        // Verifique se o modelo foi encontrado e não foi excluído
        if (!$resource || $resource->trashed()) {
            // Gerar um log
            $codigo = 404;
            $mensagem = "O ID da cor de cabelo informada não existe ou foi excluída.";
    ...
    {
        $query = RefCabeloCor::where('nome', $request->input('nome'));
        ...
        if ($query->exists()) {
            $codigo = 409;
            $mensagem = "O nome da cor de cabelo informada já existe.";
