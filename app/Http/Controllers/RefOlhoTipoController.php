...
class RefOlhoTipoController extends Controller
{
    /**
    ...
    {
        $this->authorize('update', RefOlhoTipo::class);
    ...
    {
        $this->authorize('delete', RefOlhoTipo::class);
        ...
        $response = RestResponse::createSuccessResponse([], 204, 'Tipo de olhos excluída com sucesso.');
        return response()->json($response->toArray(), $response->getStatusCode());
    }

    private function buscarRecurso($id)
    {
        $resource = RefOlhoTipo::find($id);

        // Verifique se o modelo foi encontrado e não foi excluído
        if (!$resource || $resource->trashed()) {
            // Gerar um log
            $codigo = 404;
            $mensagem = "O ID do tipo de olhos informado não existe ou foi excluído.";
    ...
    {
        $query = RefOlhoTipo::where('nome', $request->input('nome'));
        ...
        if ($query->exists()) {
            $codigo = 409;
            $mensagem = "O nome do tipo de olhos informado já existe.";
