...
class RefGeneroController extends Controller
{
    ...
    public function indexSearchAll(Request $request)
    {
        ...
        $resource = RefGenero::where('nome','LIKE', '%'. $request->input('text') .'%')
        ->orderBy('nome')->get();
        $response = RestResponse::createSuccessResponse($resource, 200);
        return response()->json($response->toArray(), $response->getStatusCode());
    }
    ...
    {
        $this->authorize('store', RefGenero::class);
    ...
    {
        $this->authorize('update', RefGenero::class);
    ...
    {
        $this->authorize('delete', RefGenero::class);
        ...
        $response = RestResponse::createSuccessResponse([], 204, 'Gênero excluído com sucesso.');
        return response()->json($response->toArray(), $response->getStatusCode());
    }

    private function buscarRecurso($id)
    {
        $resource = RefGenero::find($id);
    ...
    {
        $query = RefGenero::where('nome', $request->input('nome'));
        ...
        if ($query->exists()) {
            $codigo = 409;
            $mensagem = "O nome do gênero informado já existe.";
