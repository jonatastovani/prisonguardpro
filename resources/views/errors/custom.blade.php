@extends('site.layout')
@php
    $titulo = !empty($httpStatus) ? $httpStatus : '';
@endphp
...
@section('conteudo')
    <div class="row mt-5">
        <div class="alert alert-danger" role="alert">
            ...
            @endif
            @if ($traceId)
                <p>Reporte este código ao desenvolvedor caso o problema persista: <b>{{ $traceId }}</b></p>
            @endif
            <hr>
            <p class="mb-0">HTTP Status: {{ $httpStatus }}</p>
        </div>
        <a href="{{ URL::previous() }}" class="btn btn-primary">{{ $textButton ?? 'Voltar à página anterior' }}</a>
    </div>
    {{-- 
    <div class="row">
