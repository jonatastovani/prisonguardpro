<?php
    ...
    $permissoesNecessarias = array(9,46);
    ...
    if(empty($ordens)){
        echo '<h1 style="margin-top: 100px;">Nenhum ID de Ordem de Saída foi informado</h1>';
        exit();
    }

    $idordens = explode(',', $ordens) ;
    if(count($idordens)>1){
        $nomearquivo = "OrdemSaida-".date('YmdHis').".pdf";
    }else{
        $nomearquivo = '';
    }
    ...
    if($conexaoStatus===true){
        ...
        foreach($idordens as $idordem){
            ...
            if(count($resultado)){
                ...
                $dadosdiretor = buscaDadosDiretor(1,$data);
                ...
                <table style="font-size: 12pt; border-collapse: collapse; width: 100%;">
                    ...
                    <p class="align-cen padding-margin-0" style="font-size: 18pt; text-decoration: underline; font-weight: bolder;">Ordem de Saída nº <?=$ordem?></p>
                    
                    <p class="align-rig"><b><?=$dataextensa?></b></p>

                    <p class="indent align-jus" style="font-size: 12pt;">Pela presente Ordem de Saída, autorizo o(s) detento(s) abaixo qualificados(s), deixar esta Unidade Prisional, na data de hoje, com as cautelas de praxe, devidamente escoltado(s), conforme motivo abaixo descriminado:</p>
                        ...
                        <tr>
                            <td style="border: none;">
                                <table style="font-size: 12pt; border-collapse: collapse; width: 100%; line-height: 1em; border: 2px solid black;">
                                    ...
                                    <tr>
                                        <td class="sem-borda" colspan="2" style="padding-left: 20px;">Matrícula: <span style="font-weight: bolder;"><?=$matricula?></span></td>
                                        <td class="sem-borda" colspan="5" style="padding-left: 20px; font-size: 9pt;">Pai: <b><?=$pai?></b></td>
                                    </tr>
                                    <tr> <?php
                                        if($seguro==1 ||$trabalho==1){ ?>
                                            <td class="sem-borda matricula-destaque align-cen" colspan="2" style="padding-left: 20px; width: 30%;">SEGURO<b></b></td> <?php
                                        }else{ ?>
                                            <td class="sem-borda" style="" colspan="2"></td> <?php
                                        }
                                        ?>
                                        <td class="sem-borda" colspan="5" style="padding-left: 20px; font-size: 9pt;">Mãe: <b><?=$mae?></b></td>
                                    </tr>
                                    <tr>
                                        <td class="sem-borda" colspan="1" style="padding-left: 20px;">Raio: <b><?=$raio?></b></b></td>
                                        <td class="sem-borda" colspan="1" style="padding-left: 20px;">Cela: <b><?=$cela?></b></td>
                                        <td class="sem-borda" colspan="4" style="padding-left: 20px; font-size: 9pt; width: 50%;">Artigo(s): <b><?=$artigos?></b></td>
                                ...
                                <div style="position: absolute; bottom: 0px;">
                                    <p>Saída prevista às <b><?=$horasaida?>h<?=$minutosaida?>.</b></p>
                                    ...
                                    <p>A Escolta não poderá conduzir o(s) detento(s) a outro lugar, sendo qualquer irregularidade, comunicar responsável para providências.</p>
                                </div>
                            </td>
                        </tr>
                    </table>
