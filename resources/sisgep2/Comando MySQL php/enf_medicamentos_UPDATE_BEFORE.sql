DROP TRIGGER IF EXISTS enf_medicamentos_UPDATE_BEFORE;

delimiter $
CREATE TRIGGER enf_medicamentos_UPDATE_BEFORE
BEFORE UPDATE ON enf_medicamentos
FOR EACH ROW
...
BEGIN
    ...
    IF valorAntigo <> valorNovo THEN
		INSERT INTO enf_medicamentos_log (IDREFERENCIA, CAMPOATUALIZADO, ANTIGO, NOVO, IDATUALIZACAO, DATAATUALIZACAO, IPATUALIZACAO)
        VALUES (OLD.ID, 'NOME', valorAntigo, valorNovo, NEW.IDATUALIZACAO, NEW.DATAATUALIZACAO, NEW.IPATUALIZACAO);
    END IF;
    
    -- IDUNIDADE
    IF NEW.IDUNIDADE IS NULL THEN SET valorNovo = '**VAZIO**'; ELSE  SET valorNovo = NEW.IDUNIDADE; END IF;
    IF OLD.IDUNIDADE IS NULL THEN SET valorAntigo = '**VAZIO**'; ELSE  SET valorAntigo = OLD.IDUNIDADE; END IF;
    IF valorAntigo <> valorNovo THEN
		INSERT INTO enf_medicamentos_log (IDREFERENCIA, CAMPOATUALIZADO, ANTIGO, NOVO, IDATUALIZACAO, DATAATUALIZACAO, IPATUALIZACAO)
        VALUES (OLD.ID, 'IDUNIDADE', valorAntigo, valorNovo, NEW.IDATUALIZACAO, NEW.DATAATUALIZACAO, NEW.IPATUALIZACAO);
    END IF;
    ...
    -- QTDESTOQUE
    IF NEW.QTDESTOQUE IS NULL THEN SET valorNovo = '**VAZIO**'; ELSE  SET valorNovo = NEW.QTDESTOQUE; END IF;
    IF OLD.QTDESTOQUE IS NULL THEN SET valorAntigo = '**VAZIO**'; ELSE  SET valorAntigo = OLD.QTDESTOQUE; END IF;
    IF valorAntigo <> valorNovo THEN
		INSERT INTO enf_medicamentos_log (IDREFERENCIA, CAMPOATUALIZADO, ANTIGO, NOVO, IDATUALIZACAO, DATAATUALIZACAO, IPATUALIZACAO)
        VALUES (OLD.ID, 'QTDESTOQUE', valorAntigo, valorNovo, NEW.IDATUALIZACAO, NEW.DATAATUALIZACAO, NEW.IPATUALIZACAO);
    END IF;
