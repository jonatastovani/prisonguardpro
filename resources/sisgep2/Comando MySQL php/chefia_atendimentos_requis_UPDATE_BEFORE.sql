DROP TRIGGER IF EXISTS chefia_atendimentos_requis_UPDATE_BEFORE;

delimiter $
CREATE TRIGGER chefia_atendimentos_requis_UPDATE_BEFORE
BEFORE UPDATE ON chefia_atendimentos_requis
FOR EACH ROW
...
BEGIN
    ...
    AND 17 NOT IN (SELECT IDSITUACAO FROM chefia_atendimentos WHERE IDREQ = OLD.ID) THEN
		...
		IF valorAntigo <> valorNovo THEN
			INSERT INTO chefia_atendimentos_requis_log (IDREFERENCIA, CAMPOATUALIZADO, ANTIGO, NOVO, IDATUALIZACAO, DATAATUALIZACAO, IPATUALIZACAO)
