DROP TRIGGER IF EXISTS tab_diretorestipos_UPDATE_BEFORE;

delimiter $
CREATE TRIGGER tab_diretorestipos_UPDATE_BEFORE
BEFORE UPDATE ON tab_diretorestipos
FOR EACH ROW
...
BEGIN
    ...
    IF valorAntigo <> valorNovo THEN
		INSERT INTO tab_diretorestipos_log (IDREFERENCIA, CAMPOATUALIZADO, ANTIGO, NOVO, IDATUALIZACAO, DATAATUALIZACAO, IPATUALIZACAO)
        VALUES (OLD.ID, 'NOME', valorAntigo, valorNovo, NEW.IDATUALIZACAO, NEW.DATAATUALIZACAO, NEW.IPATUALIZACAO);
    END IF;
    
    -- SIGLA
    IF NEW.SIGLA IS NULL THEN SET valorNovo = '**VAZIO**'; ELSE  SET valorNovo = NEW.SIGLA; END IF;
    IF OLD.SIGLA IS NULL THEN SET valorAntigo = '**VAZIO**'; ELSE  SET valorAntigo = OLD.SIGLA; END IF;
    IF valorAntigo <> valorNovo THEN
		INSERT INTO tab_diretorestipos_log (IDREFERENCIA, CAMPOATUALIZADO, ANTIGO, NOVO, IDATUALIZACAO, DATAATUALIZACAO, IPATUALIZACAO)
        VALUES (OLD.ID, 'SIGLA', valorAntigo, valorNovo, NEW.IDATUALIZACAO, NEW.DATAATUALIZACAO, NEW.IPATUALIZACAO);
    END IF;
