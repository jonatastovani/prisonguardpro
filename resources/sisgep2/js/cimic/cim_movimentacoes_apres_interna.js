...
function verificaAcao(){
    ...
    if($('#incluir').prop('checked')){
        $('#selectapres').attr("disabled", "disabled");
        $('#searchapres').attr("disabled", "disabled");
        $('#titulo').html('Incluir Apresentação Interna');
        $('#impr_entrada_presos').attr("hidden", "hidden");
        $('#impr_digitais_presos').attr("hidden", "hidden");
        acao = 'incluir';
    }else{
        $('#selectapres').removeAttr("disabled");
        $('#searchapres').removeAttr("disabled").focus();
        $('#impr_entrada_presos').removeAttr("hidden");
        $('#impr_digitais_presos').removeAttr("hidden");
        $('#titulo').html('Alterar Apresentação Interna');
        acao = 'alterar';
    }
    atualizarListasApresentacoesInterna();
}
...
function adicionarPreso(idbanco, idpreso){
    ...
    if(idbanco==0){
        ...
        eventoBotaoImprimir('#impapres'+novoID,[{get:'documento',valor:['oficio_apresentacao']},{get:'apresentacoes',valor:[idbanco]},{get:'opcaocabecalho',valor:[2]},{get:'query',valor:[3]}]);
        eventoBotaoImprimir('.imp_tudo',[{get:'documento',valor:['oficio_apresentacao']},{get:'apresentacoes',valor:[idbanco]},{get:'opcaocabecalho',valor:[2]},{get:'query',valor:[3]}]);
    }
    ...
    $.ajax({
        ...
        if(result.MENSAGEM){
            ...
            result.forEach(linha => {
                ...
                $('#listaapresentacoes').append(option);            
            });
        }
    });
    $('#selectapres').html($('#listaapresentacoes').html());    
}
...
function eventoExcluirApresentacaoInterna(botao){
    botao.click(()=>{
        if(confirm('Confirma a exclusão desta Apresentação Interna?\r\r***ATENÇÃO***\rEsta ação não poderá ser desfeita!')==true){
            let idapres = $('#selectapres').val();
            if(idapres!=0 && idapres!=null && idapres!=undefined){
                excluirApresentacaoInterna(idapres);
            }
        }
    })
}

function excluirApresentacaoInterna(idapres){
    $.ajax({
        url: 'ajax/inserir_alterar/cim_movimentacoes_apresentacoes.php',
        method: 'POST',
        data: {tipo:4, idapres: idapres},
        //json é uma linguagem que ambos se entendem. Tanto javascript quanto PHP.
        dataType: 'json',
        async: false
    }).done(function(result){
...
function buscarDadosOrdemSaida(idapres){
    ...
    $.ajax({
        ...
        if(result.MENSAGEM){
            ...
            $('#datasaida').val(result[0].DATASAIDA);            
            $('#selectdestino').val(result[0].IDDESTINO).trigger('change');
            
            $('#botoesimpressao').addClass('ferramentas').append('<button class="imp_tudo temp">Imp Todos Docs</button>');

            $('#botoesimpressao').append('<button class="btn-excluir temp margin-espaco-esq">Excluir Apresentação</button>');
            eventoExcluirApresentacaoInterna($('#botoesimpressao').find('.btn-excluir'));
...
$('#selectapres').change(function(){
    var id = $('#selectapres').val();
    if(id!=0){
        $('#searchapres').val(id);
        limparCampos(false)
...
$('#searchapres').change(function(){
    var id = $('#searchapres').val();
    
    if(id!=$('#selectapres').val()){
        if(id!=0){        
        $.ajax({
            url: 'ajax/consultas/cim_busca_dados_apresentacoes.php',
            method: 'POST',
            //Insere os dados no data. key: value. Pode fazer o data fora daqui e depois inserir a variável para atribuir no data.
            data: {tipo:6, idapres: id},
            //json é uma linguagem que ambos se entendem. Tanto javascript quanto PHP.
            dataType: 'json',
            async: false
            }).done(function(result){
            ...
            if(result.MENSAGEM){
                ...
                $('#selectapres').val(result[0].VALOR).trigger('change');
            }
        });
        }else{
            $('#selectapres').val(0);
...
function salvar(){
    ...
    if(verificacao===true){
        ...
        if(idalterar!=0){
            $('#selectapres').val(idalterar).trigger('change')    
        }
    }
}
...
function verificaCampos(){
    ...
    if($('#selectapres').val()==0 && acao=='alterar'){
...
function salvarDados(){
    ...
    let iddestinoapres = $('#selectdestino').val();
    let presos = [];
...
if(idbuscar>0){
    ...
    verificaAcao();
    $('#selectapres').val(idbuscar).trigger('change');
}else{
    verificaAcao();
    $('#incluir').focus();
}
