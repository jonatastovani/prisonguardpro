...
export class modalCadastroEstadoCivil {
    ...
    */
    ...
    constructor() {
        this.#urlApi = urlRefEstadoCivil;
        this.#idModal = "#modalCadastroEstadoCivil";
    ...
    #addEventsDefault() {
        ...
        const self = this;
        const modal = $(self.#idModal);
        commonFunctions.eventDefaultModals(self, { formRegister: true, inputsSearchs: modal.find('.inputActionSearchModalCadastroEstadoCivil') });

        modal.find(".btnNewRegister").on("click", () => {
            self.#action = enumAction.POST;
            modal.find('.register-title').html('Novo Estado Civil');
    ...
    async #getRecurse() {
        ...
        try {
            ...
            if (response.data) {
                ...
                form.find('.register-title').html(`Editar Estado Civil: ${response.data.id} - ${response.data.nome}`);
            }
        } catch (error) {
            console.error(error);
            const traceId = error.traceId ? error.traceId : undefined;
    ...
    async #delButtonAction(idDel, nameDel, button = null) {
        ...
        try {
            ...
            obj.setTitle = 'Confirmação de exclusão de Estado Civil';
            obj.setMessage = `Confirma a exclusão do Estado Civil <b>${nameDel}</b>?`;
    ...
    async #delRecurse(idDel) {
        ...
        if (obj.setAction(enumAction.DELETE)) {
            ...
            try {
                ...
                commonFunctions.generateNotification(`Estado Civil deletado com sucesso!`, 'success');
                self.#promisseReturnValue.refresh = true;
