...
export class modalCadastroCabeloTipo {
    ...
    */
    ...
    constructor() {
        this.#urlApi = urlRefCabeloTipo;
        this.#idModal = "#modalCadastroCabeloTipo";
    ...
    #addEventsDefault() {
        ...
        const self = this;
        const modal = $(self.#idModal);
        commonFunctions.eventDefaultModals(self, { formRegister: true, inputsSearchs: modal.find('.inputActionSearchModalCadastroCabeloTipo') });

        modal.find(".btnNewRegister").on("click", () => {
            self.#action = enumAction.POST;
            modal.find('.register-title').html('Novo Tipo de Cabelo');
    ...
    async #getRecurse() {
        ...
        try {
            ...
            if (response.data) {
                ...
                form.find('.register-title').html(`Editar Tipo de Cabelo: ${response.data.id} - ${response.data.nome}`);
            }
        } catch (error) {
            console.error(error);
            commonFunctions.generateNotification(error.message, 'error');
    ...
    async #delButtonAction(idDel, nameDel, button = null) {
        ...
        try {
            ...
            obj.setTitle = 'Confirmação de exclusão de Tipo de Cabelo';
            obj.setMessage = `Confirma a exclusão do Tipo de Cabelo <b>${nameDel}</b>?`;
    ...
    async #delRecurse(idDel) {
        ...
        if (obj.setAction(enumAction.DELETE)) {
            ...
            try {
                ...
                $.notify(`Tipo de Cabelo deletado com sucesso!`, 'success');
                self.#promisseReturnValue.refresh = true;
