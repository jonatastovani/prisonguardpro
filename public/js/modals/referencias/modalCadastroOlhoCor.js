...
export class modalCadastroOlhoCor {
    ...
    */
    ...
    constructor() {
        this.#urlApi = urlRefOlhoCor;
        this.#idModal = "#modalCadastroOlhoCor";
    ...
    #addEventsDefault() {
        ...
        const self = this;
        const modal = $(self.#idModal);
        commonFunctions.eventDefaultModals(self, { formRegister: true, inputsSearchs: modal.find('.inputActionSearchModalCadastroOlhoCor') });

        modal.find(".btnNewRegister").on("click", () => {
            self.#action = enumAction.POST;
            modal.find('.register-title').html('Nova Cor de Olhos');
    ...
    async #getRecurse() {
        ...
        try {
            ...
            if (response.data) {
                ...
                form.find('.register-title').html(`Editar Cor de Olhos: ${response.data.id} - ${response.data.nome}`);
            }
        } catch (error) {
            console.error(error);
            const traceId = error.traceId ? error.traceId : undefined;
    ...
    async #delButtonAction(idDel, nameDel, button = null) {
        ...
        try {
            ...
            obj.setTitle = 'Confirmação de exclusão de Cor de Olhos';
            obj.setMessage = `Confirma a exclusão da Cor de Olhos <b>${nameDel}</b>?`;
    ...
    async #delRecurse(idDel) {
        ...
        if (obj.setAction(enumAction.DELETE)) {
            ...
            try {
                ...
                commonFunctions.generateNotification(`Cor de Olhos deletada com sucesso!`, 'success');
                self.#promisseReturnValue.refresh = true;
