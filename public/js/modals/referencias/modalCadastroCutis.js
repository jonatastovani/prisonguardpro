...
export class modalCadastroCutis {
    ...
    */
    ...
    constructor() {
        this.#urlApi = urlRefCutis;
        this.#idModal = "#modalCadastroCutis";
    ...
    #addEventsDefault() {
        ...
        const self = this;
        const modal = $(self.#idModal);
        commonFunctions.eventDefaultModals(self, { formRegister: true, inputsSearchs: modal.find('.inputActionSearchModalCadastroCutis') });

        modal.find(".btnNewRegister").on("click", () => {
            self.#action = enumAction.POST;
            modal.find('.register-title').html('Nova Cutis');
    ...
    async #getRecurse() {
        ...
        try {
            ...
            if (response.data) {
                ...
                form.find('.register-title').html(`Editar Cutis: ${response.data.id} - ${response.data.nome}`);
            }
        } catch (error) {
            console.error(error);
            $.notify(`Não foi possível obter os dados. Se o problema persistir consulte o desenvolvedor.\nErro: ${error.message}`, 'error');
    ...
    async #delButtonAction(idDel, nameDel, button = null) {
        ...
        try {
            ...
            obj.setTitle = 'Confirmação de exclusão de Cutis';
            obj.setMessage = `Confirma a exclusão da Cutis <b>${nameDel}</b>?`;
    ...
    async #delRecurse(idDel) {
        ...
        if (obj.setAction(enumAction.DELETE)) {
            ...
            try {
                ...
                $.notify(`Cutis deletada com sucesso!`, 'success');
                self.#promisseReturnValue.refresh = true;
